...
export const Route = createFileRoute("/products/garden/$garden")({
  head: ({ params }) => {
    ...
    const title = meta ? `${meta.name} — ${meta.tagline} — CHKPLT` : "Collection — CHKPLT";
    const desc = meta?.description ?? "CHKPLT products.";
    return {
      meta: [
        { title },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-6xl px-6 pt-24 pb-8">
        ...
        <div className="mt-8">
          <h1 className="font-display text-4xl sm:text-6xl md:text-7xl">{meta.name}</h1>
