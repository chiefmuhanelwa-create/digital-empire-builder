...
function PriceDecision() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (r?.sig) {
        setSig({ ...EMPTY_SIGNALS, ...r.sig });
        setInp({ ...EMPTY_INPUTS, ...r.inp });
      }
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    getLeak()
      ...
      .catch(() => {
        /* they have not run The Leak yet */
      });
  }, [getLeak]);
  ...
  const save = useCallback((nextSig: Signals, nextInp: PriceInputs) => {
    ...
    setInp(nextInp);
    try {
      localStorage.setItem(KEY, JSON.stringify({ sig: nextSig, inp: nextInp }));
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const finalPrice =
    verdict.band === "founding" && founding ? founding.price : (rec?.amount ?? null);
      ...
      <WorkspaceShell>
        <main className="mx-auto max-w-xl px-5 py-24 text-center">
          ...
          <h1 className="mt-5 text-[30px] font-black" style={{ color: INK }}>
            The Charge Gate is part of the Foundation Kit.
          </h1>
          <a
            href="/foundation"
      ...
      <main className="mx-auto max-w-3xl px-5 py-9">
        ...
        >
          ...
          <p className="mt-1.5 text-[16px] leading-relaxed" style={{ color: BODY }}>
            ...
            been helping people for years you almost certainly have one — you have just never
            written it down or asked whether you may mention it.
          </p>
          <a
            href="/apps/proof-ledger"
        ...
        <section className="mt-8">
          ...
          >
            ...
            <h3 className="mt-1.5 text-[24px] font-black leading-tight" style={{ color: INK }}>
              {verdict.headline}
            </h3>
        ...
        <section className="mt-12">
          ...
          <p className="mt-2 text-[17px] leading-relaxed" style={{ color: BODY }}>
            Three floors. The recommendation is the highest of them, never the average — a floor is
            a floor, and averaging lets your weakest input drag the price down.
          </p>

          <div className="mt-5 grid gap-4 sm:grid-cols-3">
            <Num
              label="Hours to deliver once"
              hint="Including preparation."
              value={inp.hours}
              onChange={(v) => save(sig, { ...inp, hours: v })}
            />
            <Num
              label="Your hourly rate (R)"
              hint={
                rateFromLeak
                  ? "Pulled from your Leak audit."
                  : "Run The Leak and this fills itself."
              }
              value={inp.hourlyRate}
              onChange={(v) => save(sig, { ...inp, hourlyRate: v })}
              highlight={rateFromLeak}
            />
            <Num
              label="Worth to them per year (R)"
              hint="Your honest estimate."
              value={inp.outcomeValue}
            ...
            {fl.map((f) => (
              <div key={f.id} className="rounded-xl p-4" style={{ border: `1px solid ${LINE}` }}>
                ...
                <p
                  ...
                  style={{ color: f.missing ? MUTED : BODY }}
                >
                  {f.missing ?? f.how}
                </p>
              </div>
            ...
            <div className="mt-6 rounded-2xl p-6" style={{ background: INK }}>
              ...
              {rec.mirror && (
                <div className="mt-4 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,.14)" }}>
                  <p className="text-[16px] font-bold" style={{ color: "#fff" }}>
                    {rec.mirror.headline}
                  </p>
                  <p className="mt-1 text-[15px] leading-relaxed" style={{ color: "#C7CEDA" }}>
                    {rec.mirror.body}
                  </p>
                </div>
              )}
          ...
          {rec && founding && verdict.band === "founding" && (
            ...
            >
              ...
              <button
                onClick={() => copy(founding.sentence)}
                className="mt-3 inline-flex items-center gap-1.5 text-[15px] font-semibold"
                style={{ color: BLUE }}
              >
              ...
              <p className="mt-3 text-[15px]" style={{ color: MUTED }}>
                A founding price only works if it visibly ends. Without the end condition it is just
                a low price you will struggle to raise.
              </p>
            </div>
          )}
          ...
          {rec && (
            <>
              <div className="mt-4 rounded-xl p-5" style={{ border: `1px solid ${LINE}` }}>
                ...
                <p className="mt-1.5 text-[16px] leading-relaxed" style={{ color: BODY }}>
                  {raiseTrigger(finalPrice ?? rec.amount)}
                </p>
              </div>
              ...
              <div className="mt-4 rounded-xl p-5" style={{ border: `1px solid ${LINE}` }}>
                ...
                <p
                  className="mt-2 whitespace-pre-line text-[16px] leading-relaxed"
                  style={{ color: BODY }}
                >
                  {transitionScript(offer.name, finalPrice ?? rec.amount)}
                </p>
                <button
                  onClick={() => copy(transitionScript(offer.name, finalPrice ?? rec.amount))}
                  className="mt-3 inline-flex items-center gap-1.5 text-[15px] font-semibold"
                  style={{ color: BLUE }}
                >
                  <Copy className="size-4" /> Copy this
...
function Toggle({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left flex items-start gap-3.5 rounded-xl p-4"
      style={{ border: `1px solid ${on ? BLUE : LINE}`, background: on ? TINT : "#fff" }}
    >
      <span
        className="inline-flex size-6 shrink-0 items-center justify-center rounded-md mt-0.5"
        style={{ background: on ? BLUE : "#fff", border: `1px solid ${on ? BLUE : LINE}` }}
      >
        {on ? (
          <Check className="size-4" style={{ color: "#fff" }} />
        ) : (
          <X className="size-3.5" style={{ color: MUTED }} />
        )}
      </span>
      <span>
...
function Num({
  ...
  return (
    <label className="block">
      ...
      <span className="block text-[14px]" style={{ color: highlight ? BLUE : MUTED }}>
        {hint}
      </span>
      <input
        type="number"
        ...
        style={{
          border: `1px solid ${highlight ? BLUE : LINE}`,
