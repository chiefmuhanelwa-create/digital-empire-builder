...
const CAPTURE: { key: keyof Fields; n: string; q: string; hint: string }[] = [
  {
    key: "ask",
    n: "1",
    q: "What do people already ask your advice on?",
    hint: "The DMs, the “can I pick your brain” coffees, the questions at work.",
  },
  {
    key: "skill",
    n: "2",
    q: "Your strongest skill from 10+ years of work",
    hint: "The thing you do without thinking that others struggle with.",
  },
  {
    key: "problem",
    n: "3",
    q: "A real problem you've personally solved",
    hint: "You went through it, you came out the other side. That journey is teachable.",
  },
];
...
const RATE: { key: "ms" | "ts" | "ss"; label: string; q: string }[] = [
  { key: "ms", label: "Mindset", q: "Do you believe you're allowed to charge for this?" },
  {
    key: "ts",
    label: "Toolset",
    q: "Do you have the tools to package and deliver it? (phone, a platform, a way to take payment)",
  },
...
const WEAK_MSG: Record<string, string> = {
  Mindset:
    "Mindset is your lowest. You don't doubt the knowledge — you doubt that you're allowed to charge for it. That's imposter syndrome, not reality. You weren't qualified either; you were called. Decide you're allowed, and the rest follows.",
  Toolset:
    "Toolset is your lowest. The expertise is there — you just haven't set up the way to package and sell it. A phone, one platform, one way to take payment. That's the whole toolset. Start with what's in your hand.",
  Skillset:
    "Skillset is your lowest — but you rated it honestly, which most people won't. Pick the ONE thing you're strongest at and go deep there, rather than spreading thin. Depth in one lane beats shallow in five.",
};
...
function KnowledgeAudit() {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(KEY, JSON.stringify({ fields, ms, ts, ss }));
    } catch {
      /* ignore */
    }
  }, [fields, ms, ts, ss]);
  ...
  const res = useMemo(() => {
    ...
    if (product === 0) {
      title = "A zero stops everything";
      text =
        "One of your three is at zero — and because they multiply, the whole thing is zero. You can have world-class skill and still earn nothing if your mindset or tools are at zero. Lift the zero first.";
    } else if (pct <= 33) {
      title = "The raw material is there";
      text =
        "You have real expertise — the readiness just isn't built yet. The weakest multiplier below is almost always the fastest to fix, and it lifts the whole score with it.";
    } else if (pct <= 66) {
      title = "You're closer than you think";
      text =
        "You're most of the way to a sellable offer. Close the gap on your weakest multiplier and you cross from 'I know things' into 'I have a product.'";
    } else {
      title = "Ship it — you're ready";
      text =
        "Mindset, tools and skill are all firing. You're sitting on a product you could launch this month. Stop auditing. Start packaging.";
    }
    ...
    const assets = order
      ...
      .map((k, i) => ({ tag: String(i + 1), text: fields[k].trim(), suggest: SUGGESTS[k] }));
    ...
    <Shell>
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
        <ToolHeader
          ...
          why={
            "Before you can price anything you have to name it. This finds the thing people already come to you for — the one so obvious to you that you have stopped noticing it is worth money."
          }
        />
        ...
        {step === "intro" && (
          <div className="nx-hero-orb mt-6">
            ...
            <p className="nx-body italic mt-4 text-[var(--text-dim)]">
              “Attention is better than qualification. Skills pay more than education.”
            </p>
            <button
        ...
        {step === "capture" && (
          <div className="mt-6">
            ...
            >
              <ArrowLeft className="size-4" /> Intro
            </button>
            ...
            <p className="nx-body mt-2">
              Don't overthink it. The thing you'd dismiss as “obvious” is usually the product.
            </p>
            ...
            <button
              ...
              className="cta-glow inline-flex items-center gap-2 mt-6"
            >
              Next: score your readiness <ArrowRight className="size-4" />
            </button>
          </div>
        )}
        ...
        {step === "rate" && (
          <div className="mt-6">
            ...
            <p className="nx-body mt-2">
              Mindset × Toolset × Skillset. They multiply — they don't add. A zero anywhere is a
              zero everywhere. Be honest; this is where the gap shows.
            </p>
                ...
                return (
                  <div key={row.key}>
                    ...
                    <input
                      ...
                      className="w-full accent-[var(--nx-gold)]"
                    />
                  </div>
                );
              })}
            ...
            <div className="nx-card !p-4 mt-6 text-center">
              <span className="font-display text-lg">
                {ms} × {ts} × {ss} ={" "}
                <span className="text-[var(--nx-gold-text)]">{res.product}</span> / 1000
              </span>
            </div>
            <button
              ...
              className="cta-glow inline-flex items-center gap-2 mt-6"
            >
              See my audit <ArrowRight className="size-4" />
            </button>
          </div>
        )}
        ...
        {step === "result" && (
          <div className="mt-6">
            <button
              ...
              className="inline-flex items-center gap-1 text-sm text-[var(--text-dim)] hover:text-foreground"
            >
              <ArrowLeft className="size-4" /> Edit ratings
            </button>
            <div className="rounded-2xl bg-[var(--obsidian)] p-6 text-white text-center mt-4">
              <div className="font-display text-4xl sm:text-5xl text-[var(--nx-gold-bright)]">
                {res.pct}%
              </div>
            ...
            {res.assets.length > 0 && (
              <div className="mt-5">
                ...
                <p className="text-sm text-[var(--text-dim)] mt-0.5 mb-3">
                  This is the product hiding in plain sight. One of these is your first paid offer.
                </p>
                <div className="space-y-3">
                  {res.assets.map((a) => (
                    <div key={a.tag} className="nx-card !p-4">
                      <div className="flex gap-3">
                        <span className="shrink-0 inline-flex h-7 w-7 items-center justify-center rounded-full bg-[var(--bg-card-hi)] text-sm font-bold text-[var(--nx-gold-text)]">
                          {a.tag}
                        </span>
                        <div>
            ...
            <AiCoach
              ...
              getPayload={() => JSON.stringify({ ...fields, ms, ts, ss })}
            />
            ...
            <div className="mt-6 rounded-2xl bg-[var(--bg-surface)] border border-border p-6 text-center">
              ...
              <p className="text-sm text-[var(--text-dim)] mt-1 max-w-md mx-auto">
                The path from “I just know stuff” to your first paid product — built around the
                expertise you named above.
              </p>
              <Link to="/apply" className="cta-glow inline-flex items-center gap-2 mt-4">
                Apply for the Accelerator <ArrowRight className="size-4" />
              </Link>
            </div>
            ...
            <button
              onClick={() => {
                setStep("intro");
                setFields({ ask: "", skill: "", problem: "" });
                setMs(5);
                setTs(5);
                setSs(5);
              }}
              className="mt-6 text-sm text-[var(--text-dim)] hover:text-foreground"
            >
              Run the audit again
            </button>
          </div>
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-16">
        <div className="nx-card !p-10 text-center">
          ...
          <p className="nx-body max-w-md mx-auto mt-2">
            Find the product hiding in your expertise. Get the kit to unlock this and every
            interactive app.
          </p>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <KitDownload
            generatedSlug="knowledge-audit"
            label="Print this step as a workbook"
            compact
          />
        </div>
        <ToolFooter
          slug="knowledge-audit"
          youNowHave="three things people already ask you for, ranked — and the top one is your first product."
        />
      </main>
    </Shell>
  );
