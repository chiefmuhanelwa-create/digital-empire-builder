...
export const emailMediaKit = createServerFn({ method: "POST" })
  .inputValidator((input) =>
    z.object({
      ...
      platforms: z.array(z.object({ name: z.string().max(60), followers: z.string().max(40), er: z.string().max(20) })).max(10),
      pillars: z.array(z.object({ name: z.string().max(120), desc: z.string().max(300) })).max(10),
  ...
  .handler(async ({ data }) => {
    ...
    void addToMailerLiteGroup(data.recipientEmail, process.env.MAILERLITE_GROUP_ID_BUYERS, {
      first_name: data.fullName ?? null,
    });
    ...
    const text = await render(React.createElement(MediaKitResultEmail, emailProps), { plainText: true });
