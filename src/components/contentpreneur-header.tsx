...
const NAV_LINKS = [
  { href: "/foundation", label: "Foundation Kit" },
  { href: "/accelerator", label: "Accelerator" },
  { href: "/starterkit", label: "Starter Kit" },
];
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop nav — inline, unchanged */}
        <nav className="hidden sm:flex items-center gap-6 text-[15px] font-medium text-[var(--text-dim)]">
          {NAV_LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className="hover:text-[var(--foreground)] transition-colors px-1 py-1"
            >
              {l.label}
            </a>
          ))}
        </nav>
        {/* Mobile — hamburger toggle instead of a horizontally-scrolling row
            that silently clipped the last link off-screen with no visual hint. */}
        <button
          type="button"
          aria-label={open ? "Close menu" : "Open menu"}
          aria-expanded={open}
          onClick={() => setOpen((o) => !o)}
          className="sm:hidden shrink-0 p-2 -mr-2 text-[var(--foreground)]"
        >
          {open ? <X className="size-5" /> : <Menu className="size-5" />}
        </button>
      </div>
      {open && (
        <nav className="sm:hidden border-t border-[var(--border)] bg-white px-4 py-3 flex flex-col gap-1 text-[15px] font-medium text-[var(--text-dim)]">
          {NAV_LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              onClick={() => setOpen(false)}
              className="hover:text-[var(--foreground)] transition-colors py-2.5"
            >
              {l.label}
            </a>
          ))}
    ...
    <footer className="border-t border-[var(--border)] bg-[#0F172A]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <nav className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-xs text-slate-500">
          <a
            href="https://contentpreneur.africa/about"
