...
## 2026-08-25 — Tools premium rollout + rate-card live-workspace rebuild (Hook Generator + Rate Card)

- **`overflow-hidden` on an ancestor silently breaks `position: sticky`.** The rate-card summary rail was wrapped in `ToolCanvas` (which uses `overflow-hidden` to clip its GoldGlow bloom); an overflow-clipping ancestor becomes the sticky element's scroll container, and since it has no constrained height the sticky child never pins — it just scrolled off. Fix: don't wrap sticky content in an overflow-hidden ancestor. Use page-level `overflow-x-clip` instead (Tailwind v4) — `clip` on one axis does NOT force the other axis to `auto` and does NOT create a scroll container, so sticky survives while horizontal glow bleed is still clipped. Keep bleeding glows inside self-clipping cards, not a page wrapper.
- **Sticky offset must clear the site header.** `SiteHeader` is `sticky top-0 z-40 h-16` (64px). A sticky sidebar at `top-6` tucks under it — use `top-20`. Same for in-page `scrollIntoView` anchors: add `scroll-mt-24` or the target lands behind the header.
- **A scroll-locked drawer makes its own buttons "dead."** When the mobile summary drawer sets `document.body.style.overflow = "hidden"` (via a `useEffect` on `drawerOpen`), a button that calls `setDrawerOpen(false)` then immediately `scrollIntoView()` does nothing — the effect that unlocks the body runs AFTER the handler, so the scroll fires while still locked. Fix: unlock the body synchronously in the handler (`document.body.style.overflow = ""`), then scroll inside a double `requestAnimationFrame` so it runs after the drawer has actually unmounted.
- **Accordions were the wrong pattern for this form on mobile.** One-open-at-a-time auto-closed sections (jarring), collapsed sections read as dead buttons, and content was hidden behind chevrons. Founder feedback: bad UX. Replaced with always-open titled cards, each with a gold numbered step badge (heading, not a control). Simpler and predictable beats "prevents long scroll" here.
- **Rate-card is now a LIVE two-column workspace, not a 3-screen form flow.** `computeRateCard` (pure/cheap) runs in a `useMemo` on every input change; the dark `#111` summary card recalcs in real time. Desktop = sticky right rail; mobile = fixed bottom bar showing the running total → slide-up bottom sheet (300ms transform + fading backdrop, drag handle, `env(safe-area-inset-bottom)` padding, `overscroll-contain`). Dropped the old "Calculate" button + calculating-theatre screen. The rich analytics (CPM/CPE compare, ER/tier evidence, benchmark bars, tips, full working, PDF email capture) moved to a full-width "deep dive" below, live once a rate exists. Analytics `complete` event now fires once via a `useRef` guard the first time a valid result appears (was on the calculate click).
- **New shared component `src/components/tools/brand-logos.tsx`** — real full-colour platform marks (`<BrandLogo platform={key} />`) keyed to the same `PlatformKey` strings as `rate-card-engine` PLATFORM: Instagram gradient, TikTok cyan/red offset, red YouTube, blue FB/LinkedIn, black X, red Pinterest, + neutral globe fallback so an unmapped key never renders blank. Replaced the old duplicated/blank lucide icons on the platform chips. This is the icon set for the rest of the tools rollout.
